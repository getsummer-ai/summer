...
RSpec.describe ProjectDomainAliasForm do
  ...
  context 'when the form is invalid' do
    it 'returns an error as the provided URL is empty' do
      model = described_class.new(project, user, { url: '' }).tap(&:update)
    ...
    it 'returns an error as the provided URL does not contain a domain' do
      model = described_class.new(project, user, { url: 'https://' }).tap(&:update)
    ...
    it 'returns an error as the provided URL does not contain a correct domain name' do
      model = described_class.new(project, user, { url: 'https://https://frank-task-218761.framer.app' }).tap(&:update)
      # an error comes from errors[:domain_alias]
      expect( model.errors.full_messages).to include "Url must be a valid url"
    end

    it 'returns an error as the provided URL has a domain_alias that is already assigned to another project' do
      user.projects.create!(
        name: 'Test2', protocol: 'http', domain: 'test2.com', domain_alias: '218761.framer.app'
      )

      model = described_class.new(project, user, { url: 'https://218761.framer.app' }).tap(&:update)
      expect( model.errors.full_messages).to \
        include("<b>218761.framer.app</b></br> is already taken by <b>Test2</b>")
    end

    it 'returns an error as the domain is too long' do
      model = described_class.new(project, user, { url: "https://www.#{'local' * 100}.com" })
    ...
    it 'returns an error as the url is too long' do
      model = described_class.new(project, user, { url: "https://www.#{'local' * 2}.com/#{'local' * 2000}" })
    ...
    it 'returns an error as the provided domain equals the primary domain name' do
      model = described_class.new(project, user, { url: "https://test.com/test" })
      expect( model.update).to be false
      expect( model.errors.full_messages).to \
  ...
  context 'when the form is valid' do
    it 'takes a domain and put the value into the domain_alias column' do
      model = described_class.new(project, user, { url: "https://www.local.com/test" })
